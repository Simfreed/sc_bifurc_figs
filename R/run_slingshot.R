...
datdir = "/Users/simonfreedman/data/bifurc/weinreb_2020"

pcsf   = paste(datdir,"truncated_svd_comps_0-100.npy", sep="/")
metf   = paste(datdir,"GSM4185642_stateFate_inVitro_metadata.txt", sep="/")
...
sim$spring       <- as.matrix(met_dat[cell_idxs,c('SPRING.x','SPRING.y')])
reducedDims(sim) <- SimpleList(spring = sim$spring, spring2 = sim$spring) # because i'm stupid

type_names<-met_dat[cell_idxs, 'Cell.type.annotation']
unique_types<-unique(type_names)
clust0      = which(unique_types=="Undifferentiated")
diff_clusts = which(unique_types!="Undifferentiated")
...
for (i in 1:length(curves)){
    cnm          = paste("curve",i,sep="") # e.g., ==> "curve1", "curve2", etc.
