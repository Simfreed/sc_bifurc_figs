...
dtraj     = np.load('{0}/dtraj.npy'.format(datdir))
...
axA.set_xticklabels(['{0:.1f}'.format(i) if np.mod(i,1)<1e-5 else '' for i in m1s])
...
plt.savefig('{0}/figS7_noeq.pdf'.format(figdir), bbox_inches='tight')
