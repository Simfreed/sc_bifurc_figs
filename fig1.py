...
marg_wd1 = int(0.5*fac)
marg_wd2 = 1*fac
marg_wd3 = int(0.5*fac)
marg_ht  = int(0.5*fac)
...
axB0  = plt.subplot(gs[rs[3]:rs[4]  ,   0  :cs[1]])
axBL  = plt.subplot(gs[rs[3]:rs[4]  , cs[2]:cs[5]])

axB1L   = plt.subplot(gs[rs[2]:rs[3],     0:cs[1]])
axB1ar1 = plt.subplot(gs[rs[2]:rs[3], cs[1]:cs[2]])
axB1a   = plt.subplot(gs[rs[2]:rs[3], cs[2]:cs[3]])
axB1ar2 = plt.subplot(gs[rs[2]:rs[3], cs[3]:cs[4]])
axB1b   = plt.subplot(gs[rs[2]:rs[3], cs[4]:cs[5]])


axB2L   = plt.subplot(gs[rs[4]:rs[5],     0:cs[1]])
axB2ar1 = plt.subplot(gs[rs[4]:rs[5], cs[1]:cs[2]])
axB2a   = plt.subplot(gs[rs[4]:rs[5], cs[2]:cs[3]])
axB2ar2 = plt.subplot(gs[rs[4]:rs[5], cs[3]:cs[4]])
axB2b   = plt.subplot(gs[rs[4]:rs[5], cs[4]:cs[5]])

#rsb  = np.hstack([[0],rs])
rsC = [0, rs[0],rs[3],rs[5]]
...
axA.set_xticks([])
axA.set_yticks([])
axA.imshow(cell_pics[6],extent=[-0.1,1.1,0,1.08],clip_on=False)
axA.set_xlim(0,1)
axA.set_ylim(0,1)
axA.axis('off')
...
# legends
labs = ['external stimuli','differentiated B', 'differentiated A',  'pluripotent']
pics = cell_pics[3::-1]
npics = len(pics)
...
ymarg3 = 0.5
ymarg4 = 0.75
imht   = imwd*1.5 #2.5 #imwd*aspect_ratio
imspcy = (ymax - (ymarg1 + ymarg2 + ymarg3 + ymarg4 + imht*npics)) / 3

rect = mpatches.FancyBboxPatch((xmarg1, ymarg1),
                               xmax-xmarg3-xmarg1, ymax-ymarg3-ymarg1,
                               facecolor='lightgrey',edgecolor='k',clip_on=False,linewidth=0.1,
...
for i in range(npics):
    axBL.imshow(pics[i],clip_on=True, extent = [x0,x0+imwd,y0[i],y0[i]+imht],zorder=1,
                aspect='auto',origin='upper')
    axBL.text(s=labs[i],x=x0+imsz+imspcx, y=y0[i]+imht/2,fontsize=6,verticalalignment=valigns[i],zorder=1)
...
for i in range(len(axs)):
    ...
    ax.annotate("", xy=(xs[i]++dxs[i], ys[i]+dys[i]), xytext=(xs[i], ys[i]),
                arrowprops=dict(headwidth=6, headlength=4, width=2, fc='white', clip_on=False), clip_on=False)
...
####### captions #######
axAc.text(s='A',x=0,y=3,fontsize=14,verticalalignment='top',horizontalalignment='left')
axAc.axis('off')

axBc.text(s='B',x=0,y=1.5,fontsize=14,verticalalignment='top',horizontalalignment='left')
axBc.axis('off')

axCc.text(s='C',x=1.5,y=3,fontsize=14,verticalalignment='top',horizontalalignment='right')
axCc.axis('off')
